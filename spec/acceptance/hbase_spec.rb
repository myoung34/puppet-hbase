...
describe 'hbase class' do
  ...
  it 'should allow port change' do
    ...
    shell("curl -s -X PUT 'http://localhost:8000/test2/schema' -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{\"@name\":\"test2\",\"ColumnSchema\":[{\"name\":\"data\"}]}'  -w '%{http_code}'") do |result|
      assert_match result.stdout, '201', 'HBase REST Master create the table.'
    end

    shell("curl -s -o /tmp/hbase_table_list2.html -w '%{http_code}' http://localhost:8000") do |result|
      assert_match result.stdout, '200', 'HBase REST Master did not return a 200 OK'
    end
